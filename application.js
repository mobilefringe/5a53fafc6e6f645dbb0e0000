...
function renderHomeHours(container, template, collection){
    ...
    $.each( item_list , function( key, val ) {
        ...
        console.log(var open_time = moment(val.open_time).tz(getPropertyTimeZone());
            var close_time = moment(val.close_time).tz(getPropertyTimeZone()););
        var rendered = Mustache.render(template_html,val);
        item_rendered.push(rendered);
    });
    ...
    $.each(collection, function(key, val) {
        if(type == "stores" || type == "category_stores"){
            if(!val.store_front_url_abs ||  val.store_front_url_abs.indexOf('missing.png') > -1 || val.store_front_url_abs.length === 0){
                ...
                val.hide_img = "display:none;"
                val.show_storename = "display:block";
            } 
            else {
                 val.hide_img = "display:block"
                 val.show_storename = "display:none";
            }
        }
